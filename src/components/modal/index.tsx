...
export default (props: IModalPropTypes) => {
  ...
  let { animationType = "none", animationDuration = 300 } = props;
  ...
  return (
    <View style={[styles.absolute, styles.wrap, wrapStyle]}>
      <TouchableWithoutFeedback onPress={_onMaskClose}>
        <Animated.View style={[styles.absolute, { opacity: opacity }]}>
          <View style={[styles.absolute, styles.mask, maskStyle]} />
        </Animated.View>
      </TouchableWithoutFeedback>
      <Animated.View style={[style, _animationStyleMap[animationType]]}>
        {children}
      </Animated.View>
    </View>
  );
